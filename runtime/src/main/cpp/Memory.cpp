...
typedef KStdUnorderedMap<void**, std::pair < KRef * , int>>
KThreadLocalStorageMap;
...
void garbageCollect(MemoryState* state, bool force) {
    ...
    GC_LOG("GC: gcToComputeRatio=%f duration=%lld sinceLast=%lld\n",
           double(gcEndTime -gcStartTime) / (gcStartTime - state->lastGcTimestamp + 1), (gcEndTime - gcStartTime),
